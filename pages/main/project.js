		...
		<div>
			...
			<div className={styles.body}>
				<div>
					...
					<span style={icons} class="material-icons">chevron_right</span>
					<span className={styles.title}>프로젝트</span>
				</div>
				<div className={styles.desc}>
					<span>
						개발을 통해 작은 문제를 해결하고 개선한<br />프로젝트들을 정리 했어요
					</span>
				</div>
				<div className={styles.companyPjt}>
					...
					<div className={classNames({ [styles.companyPjtDesc]: true, [styles.desc]: true,})}>
						<span>
							구성원으로서 작은 성과를 냈던 프로젝트 입니다.<br />
							보안상 간략하게 내용을 담았습니다.
						</span>
					</div>
					<div>
						<ul className={styles.companyListBox}>
							<li>
								...
								<div className={styles.link}>
									<Link href="https://dfcat.net"><a target="_blank">링크 &gt;</a></Link>
								</div>
							</li>
							<li>
								<div className={styles.intro}>
							...
							<li>
								...
								<div className={styles.link}>
									<Link href="https://play.google.com/store/apps/details?id=kr.co.datagen.gs.genservice"><a target="_blank">링크 &gt;</a></Link>
								</div>
							</li>
							<li>
								<div className={styles.intro}>
						...
						<ul className={styles.personalListBox}>
						  <div className={styles.buttonPrev}>
							...
							<div className={styles.liBox}>
								{list.map((list) => (
									<li key={list.id}>
										<Link href="https://naver.com"><a target="_blank">
											<div className={styles.intro}>
												<div className={styles.logo}>
...
project.getInitialProps = async () => {
	let list = []
	const response = await axios.get('http://localhost:3000/api/blog/intro')
	const { data } = response
	console.log(data)
...
function createData(id, title, skills, date, contents) {
  return { id, title, skills, date, contents };
}
